...
PGL.prototype = {
  ...
  showPopup: function (dialog, callback, css) {
    ...
    $(document.body).block({
      message: dialog, css: css, overlayCSS: { cursor: 'default' }, centerX: false, centerY: false, onUnblock: function () {
        dfd.reject();
      }
    });
  ...
  wakeup: function () {
    var that = this;
    that.showDialog(PGL.Text.get('pgltop.wakeup.confirm'), {
      ok: function () {
        that.getApi('pgl.member.profile.pdw_end_by_pgl', {}, function () {
          that.showDialog(PGL.Text.get('pgltop.wakeup.complete'), {
            ok: function () {
              location.reload();
            }
          });
